...
def execute(filters=None):
	...
	perum_clause = ""
	if filters.get("perumahan") :
		perum_clause = """ 
		AND l.`perumahan`="{0}" """.format(filters.get("perumahan"))
	result = frappe.db.sql(""" 
		SELECT l.`source`,DATE(l.`creation`) as `posting_date`,SUM(1) FROM `tabLead`l 
		{0} {1}
		GROUP BY l.`source`,`posting_date`
		ORDER BY l.`source`
		""".format(date_clause,perum_clause))
